...
class MaskableDictRolloutBufferWithKLSamples(NamedTuple):
    observations: TensorDict
    actions: th.Tensor
    old_values: th.Tensor
    old_log_prob: th.Tensor
    ...
    returns: th.Tensor
    action_masks: th.Tensor
    ref_log_probs: th.Tensor
    ref_logits: th.Tensor
...
class MaskableDictRolloutBufferWithKL(DictRolloutBuffer):
    """
    Dict Rollout buffer used in on-policy algorithms like A2C/PPO.
    Extends the RolloutBuffer to use dictionary observations
    ...
    def reset(self) -> None:
        ...
        # Reference log prob
        self.kl_rewards = np.zeros(
            (self.buffer_size, self.n_envs))  # .to(self.device)
        # Reference log prob
        self.ref_log_probs = np.zeros(
            (self.buffer_size, self.n_envs))  # .to(self.device)
        assert isinstance(self.action_space, spaces.Discrete)

        # Logits
        self.ref_logit_dims = self.action_space.n
        self.ref_logits = np.zeros(
            (self.buffer_size, self.n_envs, self.ref_logit_dims))  # .to(self.device)

        super().reset()

    def add(self, *args, action_masks: Optional[torch.Tensor] = None,
            kl_rewards: Optional[torch.Tensor] = None,
            ref_log_probs: Optional[torch.Tensor] = None,
            ref_logits: Optional[torch.Tensor] = None, **kwargs) -> None:
        """
        :param action_masks: Masks applied to constrain the choice of possible actions.
        """
        if action_masks is not None:
        ...
        if kl_rewards is not None:
            self.kl_rewards[self.pos] = kl_rewards.reshape(
                (self.n_envs,))
        if ref_log_probs is not None:
            self.ref_log_probs[self.pos] = ref_log_probs.reshape(
                (self.n_envs,))
        if ref_logits is not None:
            self.ref_logits[self.pos] = ref_logits.reshape(
                (self.n_envs, self.ref_logit_dims))
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> MaskableDictRolloutBufferSamples:
        ...
        return MaskableDictRolloutBufferWithKLSamples(
            observations={key: self.to_torch(obs[batch_inds]) for (
                key, obs) in self.observations.items()},
            actions=self.to_torch(self.actions[batch_inds]),
            old_values=self.to_torch(self.values[batch_inds].flatten()),
            ...
            action_masks=self.to_torch(
                self.action_masks[batch_inds].reshape(-1, self.mask_dims)),
            ref_log_probs=self.to_torch(self.ref_log_probs[batch_inds].flatten()),
            ref_logits=self.to_torch(
                self.ref_logits[batch_inds].reshape(-1, self.ref_logit_dims)),
        )

    def compute_returns_and_advantage_with_kl(self, last_values: th.Tensor, dones: np.ndarray, kl_coeff: float) -> None:
        super().compute_returns_and_advantage(last_values, dones)
        # Note the definition of advantage is different in this algorithm
        self.advantages = self.advantages - self.kl_rewards
