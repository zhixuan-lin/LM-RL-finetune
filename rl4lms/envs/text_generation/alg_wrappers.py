...
from rl4lms.algorithms.common.maskable.buffers import MaskableDictRolloutBuffer, MaskableDictRolloutBufferWithKL
...
class TransitionInfo:
    ...
    info: Dict[str, Any]
    ref_logit: Optional[np.ndarray] = None
...
def wrap_onpolicy_kl_alg(
    ...
    class OnPolicyAlgText(alg_class, OnPolicyWarmStartMixin):
        def __init__(
            ...
            self._analytic_kl_reward = analytic_kl_reward
            # flattened rollout buffer
            self.rollout_buffer = MaskableDictRolloutBufferWithKL(
        ...
        def generate_batch(
            ...
            for actions_tensor, _, action_mask in zip(
                ...
                with torch.no_grad():
                    ...
                    raw_log_probs, log_probs, policy_past_state, dist = (
                    ...
                    # compute KL rewards
                    if self._analytic_kl_reward:
                        # Later when computing advantage in PPOKL, this will 
                        # be overwritten
                        kl_div = torch.distributions.kl_divergence(dist, ref_dist)
                    else:
                ...
                for env_ix in range(self.env.num_envs):
                    ...
                    if not ep_terminated[env_ix]:
                        transtion = TransitionInfo(
                            ...
                            info=infos[env_ix],
                            ref_logit=ref_dist.logits[env_ix].cpu().numpy()
                        )
                ...
                for transition_ix, transition in enumerate(transitions):
                    ...
                    if not rollout_buffer.full:
                        rollout_buffer.add(
                            ...
                            kl_rewards=transition.kl_reward,
                            ref_log_probs=transition.ref_log_prob,
                            ref_logits=transition.ref_logit,
                        )
                    ...
                    if rollout_buffer.full and not advantages_computed:
                        ...
                        if self._norm_reward:
                            assert False, 'This does not work here'
                        ...
                        # Note we use a special method here
                        rollout_buffer.compute_returns_and_advantage_with_kl(
                            last_values=next_values, dones=transition.done, 
                            kl_coeff=self._kl_controller.kl_coeff
                        )
    ...
    alg = OnPolicyAlgText(alg_kwargs, kl_coeff, tracker, target_kl, norm_reward, analytic_kl_reward)
    return alg
...
def wrap_onpolicy_alg(
    ...
    target_kl: float = None,
    norm_reward: bool = False,
    analytic_kl_reward: bool = False,   # placeholder
):
    class OnPolicyAlgText(alg_class, OnPolicyWarmStartMixin):
        def __init__(
            self,
